...
본 문서는 **Team SynergeSign**의 *SignBell 프로젝트*에서 **Git 브랜치 전략, 커밋/PR 규칙, 이슈 관리 규칙을 정리하여 팀 전체가 공유할 수 있도록 작성되었습니다.**
...
```bash
feat: 수어 일치율 계산 기능 추가 (#45)

- 수어 영상 분석 및 수어 일치율 계산 기능 구현 (ex.클래스명 추가)
- 수어 일치율 확인 페이지(파일명.html, 파일명.js) 추가
- 수어 일치율 확인 페이지(파일명.js) 상태/필터 기능 확장
- SecurityConfig에 /sign/**, /dashboard 접근 허용
```
...
3. **로컬 테스트**
   ...
   # 실행 후 실제 기능 확인 (예: 수어 일치율 계산 테스트, 수어 영상 분석 데이터 확인 등)
   ```

4. **피드백 작성 및 승인**
    - `LGTM!`, `수고하셨습니다`, `확인했습니다` 등 코멘트 + Approve
...
1. **제목 줄**
    ...
    - 예: `feat: 수어 퀴즈방 참여하기 + 수어 퀴즈방 나가기 기능 구현`
