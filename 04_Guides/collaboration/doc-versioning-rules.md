...
* **형식**: `vX.Y.Z`
    ...
    * **Y (부 버전)**: 내용 추가나 수정 반영 (새로운 섹션 추가, 기존 내용 보강 등)
    * **Z (수정 버전)**: 긴급 수정이나 사소한 변경을 반영 (오타 수정, 포맷 변경 등)

예시:

* `v1.0.0`: 최초 확정본
* `v1.0.1`: 긴급 수정(오타 수정 등)
* `v1.1.0`: 소규모 업데이트(내용 추가, 오타 수정 등)
* `v2.0.0`: 문서 구조 대폭 개편 혹은 새로운 버전의 프로젝트 기준 반영

-----

## 2\. 버전 단계 구분

* **v1.0.0 (Release/최초 확정본)**
...
* **v1.0.x (Patch/긴급 수정본)**
    ...
    * 오타 수정, 문법 교정 등 사소한 변경이 이루어진 버전
    * 문서의 전체 구조나 주요 내용에는 큰 변화 없음
...
* 모든 문서는 **commit 메시지**에 버전 변경 사유를 명확히 기재한다.
    ...
    * 예시:
        ...
        * `docs: 기획서 v1.0.0 확정본 등록`
        * `docs: 설계 문서 v1.1.0 기능 설명 추가`
        * `docs: 문서 버전 관리 규칙 v1.0.1 오타 수정`
...
* `04_Guides/`: 팀 내 **작업 가이드라인** 문서를 관리합니다.
* `05_Policies/`: 프로젝트 관련 **정책 및 절차** 문서를 관리합니다.
* `06_meeting-notes/`: 프로젝트와 관련된 모든 **회의록**을 관리합니다.
* `07_Rules/`: 코딩 컨벤션, 문서 작성 규칙 등 **프로젝트 내부 규칙** 문서를 관리합니다.
* `08_ml-development/`: ML 개발 관련 문서를 통합 관리합니다.
