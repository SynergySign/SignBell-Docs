...
```mermaid
flowchart TD
    A[개인 학습 메뉴 선택] --> B[GET /api/sign-edu - 단어 리스트 조회]
    B --> C[학습할 단어 선택]
    C --> D[GET /api/sign-edu/{signId} - 단어 상세 정보]
    D --> E[설명 영상 재생]
    E --> F{거울 모드 사용}
    F -->|예| G[웹캠 활성화]
    F -->|아니오| H[다음 단어 선택]
...
```mermaid
flowchart TD
    A[실시간 퀴즈 메뉴] --> B[POST /api/quiz/rooms - 퀴즈 방 생성]
    B --> C[방 설정 완료]
    C --> D[WebSocket Secure 연결 - wss://.../ws]
    D --> E[방 대기실]
    E --> F{참여자 확인}
    F -->|2명 미만| E
    F -->|2-4명| G[퀴즈 시작 버튼 활성화]
    G --> H[STOMP /app/room/{roomId}/quiz/start]
    H --> I[퀴즈 진행]
    I --> J[퀴즈 종료]
    J --> K[최종 순위 표시]
    K --> L{재시작/방나가기}
...
```mermaid
flowchart TD
    A[실시간 퀴즈 메뉴] --> B[GET /api/quiz/rooms - 방 리스트 조회]
    B --> C{카메라 권한 확인}
    C -->|없음| D[카메라 권한 요청]
    C -->|있음| E[방 선택]
    D --> E
    E --> F[WebSocket Secure 연결 - wss://.../ws]
    F --> G[방 대기실]
    G --> H{방장 시작}
    H -->|대기| G
    H -->|시작| I[퀴즈 진행]
    
    I --> J[문제 출제 - /topic/room/{roomId}/quiz/question]
    J --> K[STOMP /app/room/{roomId}/quiz/challenge]
    K --> L{선착순 획득}
    L -->|실패| M[다음 문제 대기]
    L -->|성공| N[화면 확대]
    N --> O[3초 카운트다운]
    O --> P[5초 동안 수어 동작]
    P --> Q[MediaPipe 좌표 추출]
    Q --> R[FastAPI AI 모델 추론]
    R --> S[STOMP /app/room/{roomId}/quiz/answer]
    S --> T{정답 여부}
    T -->|정답| U[점수 획득]
    T -->|오답| V[점수 차감]
    U --> W{모든 문제 종료}
...
**API 엔드포인트**: `GET /api/sign-edu?category={category}&page={page}&size={size}`

**진입점**: 메인 대시보드 - 개인 학습 메뉴

**주요 기능**:
- 카테고리별 학습 가능한 단어 리스트 조회 (페이징 지원)
...
**Flow**:
1. `GET /api/sign-edu/categories` - 카테고리 목록 로딩
2. `GET /api/sign-edu?page=0&size=20` - 기본 단어 리스트 로딩
3. 카테고리 선택 시 `GET /api/sign-edu?category={category}` 호출
4. 스크롤 가능한 리스트 형태로 단어 목록 표시
5. 사용자가 학습할 단어 선택
6. 선택한 단어의 상세 학습 화면으로 이동

---

#### 5.1.2 개인 학습 - 단어 상세 학습 화면

**API 엔드포인트**: `GET /api/sign-edu/{signId}`
...
**Flow**:
1. `GET /api/sign-edu/{signId}` - 단어 상세 정보 조회
2. 화면 진입 시 설명 영상 자동 재생
3. 영상 시청 및 수어 동작 설명 확인
4. [거울 모드로 연습하기] 버튼 클릭 (선택)
5. 거울 모드 활성화 시:
    - 웹캠 권한 요청 및 활성화
    - **화면이 상하 분할됨**:
...
6. [거울 모드 종료하기] 버튼으로 거울 모드 비활성화
    - 다시 원래 화면으로 복귀 (영상이 크게 표시)
7. [← 뒤로가기] 버튼으로 단어 리스트로 복귀

### 5.2 실시간 퀴즈 대기실 Flow

**API 엔드포인트**:
- `GET /api/quiz/rooms` - 방 리스트 조회
- `POST /api/quiz/rooms` - 방 생성
- `GET /api/quiz/rooms/{roomId}` - 방 상세 정보
...
**Flow**:
1. `GET /api/quiz/rooms` - 방 리스트 조회 또는 `POST /api/quiz/rooms` - 방 생성
2. 카메라 권한 확인 (없으면 입장 불가)
3. WebSocket Secure 연결 (`wss://.../ws`) → 대기실 입장
4. STOMP 구독: `/topic/room/{roomId}/participants` - 참여자 변경 알림
5. WebRTC 연결 → 모든 참여자 화면 표시
6. 방장이 시작 버튼 클릭 → `STOMP /app/room/{roomId}/quiz/start`
